...
### Changed
- **Low-risk major version bumps.** Applied the curated low-risk batch of major updates that have stable, near-drop-in APIs: `lucide-react` 0.577 → 1.14.0 (post-1.0 stable, same icon API), `tailwind-merge` 2 → 3 (still supports Tailwind 3), `sonner` 1 → 2 (toast API unchanged for our usage), `uuid` 13 → 14 (`v4` named export unchanged), `vaul` 0.9 → 1.1.2 (drawer API stable), `@hookform/resolvers` 3 → 5 (zod resolver import unchanged for our usage). Regenerated both `package-lock.json` and `bun.lock`. Medium-risk tooling bumps (vite 5→8, eslint 9→10, typescript 5.9→6, react-day-picker 8→9) and high-risk framework bumps (react 19, react-router 7, tailwind 4, zod 4) deliberately deferred.
